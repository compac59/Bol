...
/// Nos groupes musculaires -> muscles du package flutter_body_atlas à colorer.
List<atlas.MuscleInfo> musclesForGroup(engine.MuscleGroup g) {
  switch (g) {
...
/// Vue la plus pertinente pour montrer un groupe : arrière pour dos/triceps.
atlas.AtlasAsset _viewForGroup(engine.MuscleGroup g) {
  switch (g) {
    case engine.MuscleGroup.dos:
    case engine.MuscleGroup.triceps:
      return atlas.AtlasAsset.musclesBack;
    default:
      return atlas.AtlasAsset.musclesFront;
  }
}

/// Petite silhouette qui met en surbrillance **un seul** groupe musculaire,
/// à afficher à côté d'un exercice.
class MuscleThumbnail extends StatelessWidget {
  const MuscleThumbnail({super.key, required this.group, this.width = 64});

  final engine.MuscleGroup group;
  final double width;
  ...
  Widget build(BuildContext context) {
    ...
    final mapping = <atlas.MuscleInfo, Color?>{
      for (final m in musclesForGroup(group)) m: accent,
    };
    return SizedBox(
      width: width,
      height: width * 1.7,
      child: atlas.BodyAtlasView<atlas.MuscleInfo>(
        view: _viewForGroup(group),
...
/// Schéma anatomique complet (avant + arrière) mettant en surbrillance
/// plusieurs groupes — utilisable pour une vue d'ensemble.
class BodyMap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final g in worked) {
      for (final m in musclesForGroup(g)) {
        mapping[m] = accent;
      }
    }
